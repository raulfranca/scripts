...
- **Seção "Dados Pessoais" movida para o topo do formulário (`criarFormulario`):** o cabeçalho "Dados Pessoais" e a linha CPF/RG/Nacionalidade/Data de Nascimento voltaram a ser os **primeiros** elementos de `#cred-form-container` (antes ficavam no final, após "Regiões Escolares" — ver 0.6.1). Como `moverDocIdentidade` (anexo II) e `moverDocPIS` (anexo VI) injetam logo após esse cabeçalho, a ordem final passa a ser: cabeçalho "Dados Pessoais" → anexo II → anexo VI → CPF/RG/Nacionalidade/Nascimento → Estado civil → Endereço → E-mail/Celular → Conta Santander → Função → Regiões. Mudança puramente de ordem no DOM; IDs, listeners e validação inalterados.
