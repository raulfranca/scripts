...
- **Dados da Conta Santander agora são opcionais (`validarFormulario` e `_estaCompleto`):** é possível concluir ("Concluir e copiar") com os campos **Agência** e **Conta** em branco. As checagens deixaram de exigir preenchimento; passaram a validar apenas quando há conteúdo — agência precisa ter 4 dígitos (ou ficar vazia) e conta 9 dígitos (ou ficar vazia). Um valor parcialmente digitado continua bloqueando a conclusão. Mensagens de erro ajustadas para indicar que o campo pode ficar em branco.
