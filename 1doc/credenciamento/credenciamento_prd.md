...
* **Modal Nativo Modificado:** O script utiliza o modal nativo `#modal_aprovacao_anexos` (Bootstrap 2) que já existe na página, acionado pelo botão "Tabela" (`a.link_tabela_revisao_anexos`). O script clica programaticamente nesse botão, aguarda o carregamento AJAX da tabela de documentos e injeta seus controles dentro do modal. Estrutura em 5 zonas:
  ...
  * **Dois cabeçalhos de grupo dividem o modal em etapas** (`.cred-section-group-header`, injetados por `injetarHeadersSecao`):
    * **"Preenchimento de Dados Pessoais"** (`#cred-header-preenchimento`) — no `modalBody`, **acima da subseção I (Ficha de Inscrição)**. Marca o início da etapa de preenchimento dos campos do candidato.
    * **"Verificação de Documentos"** (`#cred-header-verificacao`) — logo **acima da tabela nativa de anexos** (`.div_lista_aprovacao_anexos`). Marca o início da etapa de conferência dos demais documentos (III, IV, V, VII…).
  * **Princípio de organização — "dado junto do seu documento":** cada campo preenchido pelo credenciador é lido de um anexo específico da tabela de revisão, então mora **dentro da subseção daquele anexo**, logo abaixo do arquivo que deve ser consultado. No template (`criarFormulario`) os campos nascem agrupados por id e os movers os realocam para dentro das respectivas subseções:
    * **Anexo I — Ficha de Inscrição (`#cred-ficha-inscricao`):** contém os campos gerais lidos da ficha — **Estado civil, Endereço (CEP/Logradouro/Número/Bairro/Cidade), E-mail, Celular, Dados da Conta Santander (Agência/Conta), Função pretendida, Regiões Escolares**. No template ficam agrupados sob o id `#cred-ficha-campos`, realocado por `moverFichaInscricao` para dentro da subseção I (após o anexo e o aviso).
    * **Anexo II — Documento de Identidade (`#cred-doc-identidade`):** contém **CPF, RG, Nacionalidade e Data de Nascimento**. No template ficam em `.cred-dados-row` com id `#cred-identidade-campos`, realocado por `moverDocIdentidade`.
    * **Anexo VI — PIS/PASEP/NIT (`#cred-doc-pis`):** contém o **número do PIS**, realocado por `moverDocPIS`.
    * **Ordem final das seções no modal:** cabeçalho **"Preenchimento de Dados Pessoais"** → Ficha de Inscrição (I) [+ campos gerais] → anexo II [+ CPF/RG/Nacionalidade/Nascimento] → anexo VI [+ número do PIS] → cabeçalho **"Verificação de Documentos"** → tabela nativa com o restante dos anexos.
  ...
  **Tabela de documentos (conteúdo nativo do 1Doc):** carregada via AJAX dentro de `.div_lista_aprovacao_anexos`. Exibe cada documento categorizado com nome, data e botão "Revisar" nativo. A linha "I - Ficha de Inscrição" é removida da tabela e movida para uma seção de destaque (`.cred-ficha-section`, `#cred-ficha-inscricao`) posicionada acima do restante do formulário, contendo: label da categoria, inner table com os documentos e botões Sim/Não, um aviso em fundo amarelo (*"Conferir se a ficha de inscrição é a versão retificada: 3 – Zona Leste, 4 – Moreira César."*) e, abaixo, os **campos gerais lidos da ficha** (`#cred-ficha-campos`: Estado civil, Endereço, E-mail/Celular, Conta Santander, Função, Regiões).
