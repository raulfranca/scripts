...
> 5. E-mail — se preenchido, deve passar no regex `/^[^\s@]+@[^\s@]+\.[^\s@]+$/` (opcional, mas formato obrigatório se presente).
> 6. Botões Sim/Não — todos os grupos de categoria devem ter avaliação.
...
A função `copiarParaPlanilha()` lê diretamente das variáveis de estado do módulo (sem parâmetros). Mapeamento completo das 38 colunas (A–AL):
...
| I | CEP | 8 dígitos | `cep` |
| J | Logradouro | Texto livre | `logradouro` |
| K | Número | Texto livre | `numero` |
| L | Bairro | Texto livre | `bairro` |
| M | Cidade | Texto livre | `cidade` |
| N | E-mail | Texto livre | `email` |
| O | Celular | Só dígitos | `celularDigitos` |
| P | Banco | Texto livre | `bancoNome` |
| Q | Chave Pix | Texto livre | `chavePix` |
| R | Educação Básica | `Educação Básica` ou vazio | `funcoesSelecionadas` (valor interno `Ed. Básica`) |
| S | Educação Física | `Educação Física` ou vazio | `funcoesSelecionadas` (valor interno `Ed. Física`) |
| T | Artes | `Artes` ou vazio | `funcoesSelecionadas` (valor interno `Artes`) |
| U–Y | Regiões 1–5 | Número inteiro ou vazio | `regioesSelecionadas` |
| Z–AJ | Documentos I–XI | `sim`, `não` ou vazio | `avaliacoesDocs[cat]` (true→sim, false→não, ausente→vazio) |
| AK | Resultado | `habilitado` ou `inabilitado` | Calculado: algum `false` → inabilitado; todos `true` → habilitado |
| AL | Ciclo | `'01'`–`'10'` ou vazio | `cicloAtual` (calculado por `aplicarMarcadorCiclo`) |
...
2. **Copia para o clipboard:** monta as 38 colunas (A–AL) e chama `navigator.clipboard.write` com `text/html` e `text/plain`.
